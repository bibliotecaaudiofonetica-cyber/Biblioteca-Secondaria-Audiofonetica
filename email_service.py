...
# ══════════════════════════════════════════════════════════════════════════════
# 8. RICHIESTA RESET PIN (allo studente, conferma SENZA il PIN)
# ══════════════════════════════════════════════════════════════════════════════

def build_pin_reset_request_html(student_name: str) -> str:
    body = f"""
      <p style="font-size:16px;color:#1A1208;margin:0 0 14px;">
        Ciao <strong>{student_name}</strong>,
      </p>
      <p style="font-size:15px;color:#1A1208;margin:0 0 18px;">
        Abbiamo ricevuto la tua richiesta di reimpostare il PIN di accesso
        alla biblioteca.
      </p>
      <div style="margin:18px 0;padding:18px 20px;background:#FBF6EC;
                  border-left:4px solid #C49A3C;border-radius:8px;
                  font-size:14.5px;color:#1A1208;line-height:1.6;">
        Per motivi di sicurezza il nuovo PIN <strong>non viene inviato per
        email</strong>: la bibliotecaria/il bibliotecario lo genererà e te
        lo consegnerà di persona.
      </div>
      <p style="font-size:14px;color:#8B6550;margin:0;">
        Se non hai richiesto tu il reset del PIN, parlane con l'amministratore
        della biblioteca. 📚
      </p>"""
    return _base_template("Richiesta PIN ricevuta", "🔑", "#C49A3C", body)


def send_pin_reset_request(student_name: str, student_email: str) -> bool:
    if not student_email:
        return False
    html = build_pin_reset_request_html(student_name)
    return send_email(
        to_email=student_email,
        to_name=student_name,
        subject="🔑 Richiesta di reset PIN ricevuta",
        html_body=html,
    )


# ══════════════════════════════════════════════════════════════════════════════
# 9. CONFERMA RESTITUZIONE (allo studente, con riepilogo punti se disponibile)
# ══════════════════════════════════════════════════════════════════════════════

_RETURN_REASON_LABELS = {
    "early":   ("🌟", "Restituito in anticipo!"),
    "on_time": ("✅", "Restituito in tempo!"),
    "late":    ("⏰", "Restituito in ritardo"),
}


def build_return_confirm_html(student_name: str, book_title: str, return_date_str: str,
                               gamification: dict = None) -> str:
    points_block = ""
    if gamification and gamification.get("reason") in _RETURN_REASON_LABELS:
        icon, label = _RETURN_REASON_LABELS[gamification["reason"]]
        delta = gamification.get("delta", 0)
        sign = "+" if delta >= 0 else ""
        new_score = gamification.get("newScore")
        points_block = f"""
      <div style="text-align:center;margin:20px 0;padding:18px;background:#FBF6EC;
                  border-radius:14px;border:2px solid #E8C46A;">
        <div style="font-size:32px;line-height:1;margin-bottom:6px;">{icon}</div>
        <div style="font-size:16px;font-weight:700;color:#5C3D2E;margin-bottom:4px;">{label}</div>
        <div style="font-size:14px;color:#8B6550;">{sign}{delta} punti · punteggio totale: {new_score}</div>
      </div>"""
    ...
    body = f"""
      ...
      <p style="font-size:15px;color:#1A1208;margin:0;">
        Abbiamo registrato la restituzione di questo libro:
      </p>
      {_info_table(
          ("📖 Libro",        book_title),
          ("📅 Restituito il", return_date_str),
      )}
      {points_block}
      <p style="font-size:14px;color:#8B6550;margin:0;">
        Grazie per averlo riportato in biblioteca — buona prossima lettura! 📚
      </p>"""
    return _base_template("Restituzione confermata", "📥", "#2E6B45", body)


def send_return_confirm(student_name: str, student_email: str, book_title: str,
                         return_date_str: str, gamification: dict = None) -> bool:
    if not student_email:
        return False
    html = build_return_confirm_html(student_name, book_title, return_date_str, gamification)
    return send_email(
        to_email=student_email,
        to_name=student_name,
        subject=f"📥 Restituzione confermata: {book_title}",
        html_body=html,
    )
