...
class Student(Base):
    ...
    email      = Column(String, nullable=True)  # impostata dallo studente al primo accesso (o dall'admin)
    active     = Column(Boolean, default=True, index=True)
    created_at = Column(DateTime(timezone=True), default=now_utc)
...
class Notification(Base):
    ...
    usato come chiave perché è univoco), un professore (recipient_type='teacher',
    recipient_id = Teacher.id), oppure l'amministratore (recipient_type='admin',
    recipient_id = costante 'admin' — non esiste una riga Admin nel db: il
    login admin è una singola password globale, quindi non serve un id per
    utente; le notifiche admin sono semplicemente quelle con recipient_type
    'admin', tutte visibili a chiunque accede col pannello admin).
    """
    __tablename__ = "notifications"

    id             = Column(String, primary_key=True, default=new_id)
    recipient_type = Column(String, nullable=False, index=True)  # "student" | "teacher" | "admin"
    recipient_id   = Column(String, nullable=False, index=True)  # full_name / teacher.id / "admin"
    kind           = Column(String, nullable=False)  # "loan_confirm" | "due_soon" | "overdue" |
                                                        # "teacher_alert" | "badge_earned" |
                                                        # "admin_broadcast" | "pin_reset_request"
    title          = Column(String, nullable=False)
    body           = Column(String, default="")
    # Nato per i prestiti (da cui il nome), ma è solo una stringa libera senza
    # vincolo di chiave esterna: per le richieste di reset PIN (pin_reset_request)
    # lo riusiamo per portare lo Student.id a cui la richiesta si riferisce,
    # così il pannello admin può aprire subito la scheda dell'alunno giusto.
